...
class Meta:
    def __init__(self, path_to_yaml: str):
        ...
        if path_to_yaml == "":
            print(f"Path not provided. Metadata not set.")
        else:
...
def metas_from_json(path_to_json: str):
    if os.path.exists(path_to_json):
        ...
        left_meta.distortion_coefficients = np.array(d['left']['distortion']).reshape((1,5))
        left_meta.rectification_matrix = np.array(d['left']['rectification']).reshape((3,3))
        ...
        right_meta.distortion_coefficients = np.array(d['right']['distortion']).reshape((1,5))
        right_meta.rectification_matrix = np.array(d['right']['rectification']).reshape((3,3))
        right_meta.projection_matrix = np.array(d['right']['projection']).reshape((3,4))
