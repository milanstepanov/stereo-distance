...
class Bag:
    ...
    def set_error(self, error):
        if self.metric_error is not None:
            raise ValueError(
                f"Cannot set performance error. Error already computed: {self.metric_error}."
            )
                    ...
                    for bag in data["bags"]:
                        if os.path.exists(f"{bag.path}-roi.yaml"):
                            with open(f"{bag.path}-roi.yaml", "r") as stream:
                                ...
                                bag.set_roi(
                                    content["roi"]["top-left"],
                                    content["roi"]["bottom-right"],
                                )

                    if not (
                        os.path.exists(data["camera-left"])
                        and os.path.exists(data["camera-right"])
                    ):
                        raise FileNotFoundError(
                            "Check if camera configuration files are in {dataset} and if they are named 'left.yaml' and 'right'.yaml'"
                        )
        ...
        for dataset in self.datasets:
            ...
            for bag in dataset["bags"]:
                ...
                if bag.have_roi:
                    ...
                    estimator.compute_disparity(
                        is_checkerboard=True, checkerboard_pattern=(10, 7)
                    )
...
if __name__ == "__main__":
    ...
    bench = Benchmark(["data/A008-01-27", "data/A008-02-02"])
